...
# 定义一个名为TensorVMSplit的类，它是TensorBase的子类
# 和https://github.com/apchenstu/TensoRF/blob/main/models/tensoRF.py中的 class TensorVMSplit(TensorBase)一致
class TensorVMSplit(TensorBase):
    def __init__(self, aabb, gridSize, tSize, device, **kargs):
        # 调用父类TensorBase的初始化方法，传递给它aabb、gridSize、tSize、device以及其他关键字参数
        super(TensorVMSplit, self).__init__(aabb, gridSize, tSize, device, **kargs)

    def init_svd_volume(self, res, device):
        # 初始化密度和外观的奇异值分解（SVD）参数
        # 使用init_one_svd方法初始化密度平面和线，以及外观平面和线
        ...
        # 创建一个线性模型，用于将外观特征映射到指定的维度，没有偏置项
    ...
    def init_one_svd(self, n_component, gridSize, scale, device):
        # 初始化奇异值分解的参数
        plane_coef, line_coef = [], []  # 存储平面系数和线性系数
        for i in range(len(self.vecMode)):
            vec_id = self.vecMode[i]  # 获取向量的索引
            mat_id_0, mat_id_1 = self.matMode[i]  # 获取矩阵的索引
            # 生成服从标准正态分布的随机张量，并乘以缩放因子scale
            # 注意，这里使用torch.nn.Parameter()将其定义为模型参数，以便在训练过程中进行优化
            # 对于平面系数，其形状为 (1, n_component[i], gridSize[mat_id_1], gridSize[mat_id_0])
            plane_coef.append(
                torch.nn.Parameter(
                    scale
                    * torch.randn(
            ...
            # 对于线性系数，其形状为 (1, n_component[i], gridSize[vec_id], 1)
            line_coef.append(
                torch.nn.Parameter(
                    scale * torch.randn((1, n_component[i], gridSize[vec_id], 1))
                )
            )
        # 将平面系数和线性系数封装为ParameterList，并将其移动到指定的设备上
    ...
    def get_optparam_groups(self, lr_init_spatialxyz=0.02, lr_init_network=0.001):
        # 定义优化器参数组
        grad_vars = [
            {"params": self.density_line, "lr": lr_init_spatialxyz},  # 密度线性系数
            {"params": self.density_plane, "lr": lr_init_spatialxyz},  # 密度平面系数
            {"params": self.app_line, "lr": lr_init_spatialxyz},  # 颜色线性系数
            {"params": self.app_plane, "lr": lr_init_spatialxyz},  # 颜色平面系数
            {"params": self.basis_mat.parameters(), "lr": lr_init_network},  # 基础矩阵参数
        ]
        # 如果renderModule是torch.nn.Module类型，将其参数也添加到优化器参数组中
