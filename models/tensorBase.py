...
class TensorBase(torch.nn.Module):
    ...
    def sample_ray_ndc(self, rays_o, rays_d, is_train=True, N_samples=-1):
        # 如果未指定采样点数量，则使用默认值 self.nSamples
        N_samples = N_samples if N_samples > 0 else self.nSamples
        # 获取近平面和远平面的深度值
        near, far = self.near_far
        # 在近平面和远平面之间线性插值得到 N_samples 个深度值
        interpx = torch.linspace(near, far, N_samples).unsqueeze(0).to(rays_o)
        # 如果处于训练模式，并且需要对插值结果进行扰动
        if is_train:
            # 对插值结果加上随机扰动，以增加样本的多样性
            interpx += torch.rand_like(interpx).to(rays_o) * ((far - near) / N_samples)

        # 计算射线上每个深度值对应的点坐标
        rays_pts = rays_o[..., None, :] + rays_d[..., None, :] * interpx[..., None]
        # 判断计算出的点坐标是否位于指定的边界框内部
        mask_outbbox = (
            # 判断点坐标是否小于边界框的最小坐标或大于最大坐标
            (self.aabb.to(rays_pts.get_device())[0] > rays_pts)
            | (rays_pts > self.aabb.to(rays_pts.get_device())[1])
        ).any(dim=-1)  # 沿着深度维度（即沿着每条射线）进行判断

        # 返回采样的点坐标、插值深度值和是否超出边界框的掩码
        ...
        for i in range(gridSize[0]):
            ...
            for t_idx, t in enumerate(range(self.tSize.item())):
                ...
                alpha[i, :, :, t_idx] = self.compute_alpha(
                    dense_xyz[i].view(-1, 3),  # 将坐标展平为二维
                    t / (self.tSize.item() - 1.0) * 2.0 - 1.0,  # 根据时间步长计算时间
                    self.stepSize,  # 设置步长
                ).view((gridSize[1], gridSize[2]))  # 重新整形为网格大小
        # 返回密度体积和密度采样点坐标
