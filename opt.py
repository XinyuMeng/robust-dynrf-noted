...
def config_parser(cmd=None):
    ...
    parser.add_argument(
        "--tblogdir", #存储TensorBoard日志的目录路径，默认为basedir
        type=str,
    ...
    parser.add_argument(
        "--progress_refresh_rate",#显示psnrs或迭代次数的刷新率
        type=int,
        default=10,
    ...
    parser.add_argument(
        "--dataset_name", type=str, default="nvidia", choices=["nvidia", "davis"]
    )
    ...
    parser.add_argument(
        "--distortion_weight_static", type=float, default=0.0, help="loss weight"
    )
    parser.add_argument(
        "--distortion_weight_dynamic", type=float, default=0.0, help="loss weight"
    )
    parser.add_argument(
        "--monodepth_weight_static", type=float, default=0.04, help="loss weight"
    )
    parser.add_argument(
        "--monodepth_weight_dynamic", type=float, default=0.04, help="loss weight"
    )
    parser.add_argument(
        "--smooth_scene_flow_weight", type=float, default=0.1, help="loss weight"
    )
    parser.add_argument(
        "--small_scene_flow_weight", type=float, default=0.1, help="loss weight"
    )
    ...
    parser.add_argument("--N_voxel_t", type=int, default=12)
    ...
    parser.add_argument(
        "--use_foreground_mask",
        type=str,
        default="motion_masks",
        choices=["motion_masks", "epipolar_motion_masks", "epipolar_error_png"],
    )
