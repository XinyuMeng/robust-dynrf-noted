...
#将姿态表示（通常由旋转和平移组成）转换为一个4x4的变换矩阵，其中包括了旋转和平移信息
def pose_to_mtx(pose):
    ...
    # 提取姿态中的第二个方向向量（b2）
    # b2由姿态中的第二个方向向量减去b1方向上的投影得到
    b2 = pose[..., 3:6] - torch.sum(b1 * pose[..., 3:6], dim=-1)[:, None] * b1
    # 对b2进行归一化，保证单位长度
    b2 = b2 / torch.norm(b2, dim=-1)[:, None]
    # 计算第三个方向向量（b3）为b1和b2的叉乘
    b3 = torch.cross(b1, b2)

    # 将归一化后的方向向量和平移向量按照一定顺序堆叠成矩阵
    return torch.stack([b1, b2, b3, pose[..., 6:9]], dim=-1)
