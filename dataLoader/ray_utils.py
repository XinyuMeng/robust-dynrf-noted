...
def get_rays_lean(directions, c2w):
    ...
    rays_d = torch.bmm(directions[:, None, :], torch.transpose(c2w[:, :3, :3], 1, 2))[
        :, 0, :
    ]  # (B, 3)
...
def ndc_rays_blender2(H, W, focal, near, rays_o, rays_d):
    '''
    归一化设备坐标空间（Normalized Device Coordinates，简称NDC）通常是一个规范化的立方体，其范围在每个轴上从-1到1。
    这个函数的目的是将射线的起点和方向从相机空间（世界空间）映射到这个归一化的立方体中。

    '''
    # Shift ray origins to near plane
    t = -(near + rays_o[..., 2]) / rays_d[..., 2]  # 计算每条射线与近平面的交点 t
    rays_o = rays_o + t[..., None] * rays_d  # 将射线原点沿着射线方向移动到交点位置

    # Projection
    # 计算射线在归一化设备坐标（NDC）空间中的起点坐标
    o0 = -1.0 / (W / (2.0 * focal[0])) * rays_o[..., 0] / rays_o[..., 2]
    o1 = -1.0 / (H / (2.0 * focal[1])) * rays_o[..., 1] / rays_o[..., 2]
    o2 = 1.0 + 2.0 * near / rays_o[..., 2]

    # 计算射线在归一化设备坐标（NDC）空间中的方向向量
    d0 = -1.0 / (W / (2.0 * focal[0])) * (rays_d[..., 0] / rays_d[..., 2] - rays_o[..., 0] / rays_o[..., 2])
    d1 = -1.0 / (H / (2.0 * focal[1])) * (rays_d[..., 1] / rays_d[..., 2] - rays_o[..., 1] / rays_o[..., 2])
    d2 = -2.0 * near / rays_o[..., 2]

    rays_o = torch.stack([o0, o1, o2], -1)  # 组合起点坐标
    rays_d = torch.stack([d0, d1, d2], -1)  # 组合方向向量

    return rays_o, rays_d  # 返回转换后的射线起点和方向
